...
async def execute_condition_branch(
    ...
    if mode == "switch":
        ...
        for case in cases:
            ...
            if _compare(actual, operator, value):
                ...
                log(nid, _INFO, f"switch 命中分支: {label}")
                ctx["_route"] = label
                return {"_route": label}
    ...
    true_label = node_data.get("true_label") or node_data.get("trueLabel") or "true"
    false_label = node_data.get("false_label") or node_data.get("falseLabel") or "false"
    route = true_label if satisfied else false_label
