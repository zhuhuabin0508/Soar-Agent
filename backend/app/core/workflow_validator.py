...
def _check_condition_branch_outputs(
    ...
    - if_else 模式：检查 ``true_label`` / ``false_label`` 对应出口是否连接。
    - switch 模式：遍历 ``cases`` 各 label 与 ``default`` 是否连接。
    ...
    if mode == "switch":
        ...
        # default 出口
        expected_labels.append("default")
        for label in expected_labels:
            if label not in out_handles:
                warnings.append(
    ...
    # if_else 模式
    true_label = node_data.get("true_label") or node_data.get("trueLabel") or "是"
    false_label = node_data.get("false_label") or node_data.get("falseLabel") or "否"
    if true_label not in out_handles:
        warnings.append(f"条件分支节点 {node_id} 的「是」出口未连接下游")
    if false_label not in out_handles:
        warnings.append(f"条件分支节点 {node_id} 的「否」出口未连接下游")
