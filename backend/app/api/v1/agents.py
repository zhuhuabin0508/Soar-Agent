...
def _create_llm(agent, db):
    ...
    llm_config = _load_llm_config(db, agent.model_config_id)
...
def list_agents(db: Session = Depends(get_db)) -> list[dict]:
    ...
    return to_dict_list(agents)


@router.get("/{agent_id}")
def get_agent(agent_id: int, db: Session = Depends(get_db)) -> dict:
    """获取单个智能体详情。
...
def create_agent(body: AgentBase, db: Session = Depends(get_db)) -> dict:
    """创建智能体。"""
...
@router.put("/{agent_id}")
def update_agent(agent_id: int, body: AgentBase, db: Session = Depends(get_db)) -> dict:
    """更新智能体。"""
...
async def test_agent_stream(
    ...
    async def sse_stream():
        ...
        if not has_tools:
            ...
            yield f"data: {json.dumps({'type': 'done', 'reply': full_reply}, ensure_ascii=False)}\n\n"
...
async def chat_agent(
    ...
    try:
        executor = HermesAgentExecutor(db=db, agent=agent, user=current_user)
    except Exception as exc:  # noqa: BLE001
                    ...
                    if chunk.startswith("data: "):
                        ...
                        if etype == "done":
                            ...
                            if tool_calls_detail:
                                ...
                                tool_calls_detail[-1]["finished_at"] = datetime.now().isoformat()
