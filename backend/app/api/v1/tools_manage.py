...
class ToolBase(BaseModel):
    ...
    category: str | None = Field(None, description="工具集分类：file_operations / security / cron_jobs / memory / computer_use / clarifying_question / task_planning / task_delegation")
...
def create_tool(body: ToolBase, db: Session = Depends(get_db)) -> dict:
    """创建工具。"""
...
def update_tool(tool_id: int, body: ToolBase, db: Session = Depends(get_db)) -> dict:
    """更新工具。"""
...
@router.get("/{tool_id:int}/inferred-schema")
