  ...
  return (
    <div className="rounded-lg border border-gray-800 bg-gray-900/40 p-4">
      <div className="mb-2 flex items-center gap-2">
        ...
        {name && (
          <code className="rounded bg-gray-800 px-1.5 py-0.5 text-[11px] text-brand-300">{name}</code>
        )}
      </div>
...
function Tip({ type = 'tip', children }) {
  const styles = {
    tip: 'border-brand-500/30 bg-brand-500/10 text-brand-200',
...
function Section({ id, icon, title, subtitle, children }) {
  return (
    <section id={id} className="scroll-mt-20 border-t border-gray-800/60 py-8 first:border-t-0">
      <div className="mb-4 flex items-center gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-brand-500/15 text-xl ring-1 ring-brand-500/30">
          {icon}
        </span>
        <div>
...
function AgentTutorial() {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-gray-950 text-gray-100">
      {/* 左侧目录 */}
      <aside className="hidden w-56 shrink-0 overflow-y-auto border-r border-gray-800 bg-gray-900/40 p-3 lg:block">
        <div className="mb-3 px-2 text-xs font-semibold uppercase tracking-wider text-gray-600">
          📋 目录
        </div>
        ...
        <button
          ...
          className="mt-4 w-full rounded-md border border-gray-700 px-3 py-2 text-sm text-gray-400 transition hover:bg-gray-800 hover:text-gray-200"
        >
          ← 返回智能体列表
        </button>
      </aside>
      ...
      <div ref={contentRef} className="flex-1 overflow-y-auto">
        ...
        <div className="relative h-48 overflow-hidden md:h-64">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
            <h1 className="text-2xl font-bold text-white md:text-3xl">智能体使用教程</h1>
        ...
        <div className="mx-auto max-w-4xl px-6 pb-20 md:px-10">
          ...
          <Section id="overview" icon="📖" title="智能体概述" subtitle="什么是智能体？它能做什么？">
            <p className="text-sm leading-relaxed text-gray-300">
              <strong className="text-gray-100">智能体（Agent）</strong>是 SOAR 平台的核心执行单元。
            ...
            <div className="rounded-lg border border-gray-800 bg-gray-900/60 p-4">
              <div className="mb-2 text-xs font-semibold text-gray-500">架构示意</div>
              <pre className="overflow-x-auto text-xs text-gray-300">{`用户输入
   ↓
┌──────────────────────────────────┐
...
用户看到回复 + 思考过程 + 工具调用详情`}</pre>
          ...
          <Section id="create" icon="✨" title="创建智能体" subtitle="从哪里开始？">
            <p className="text-sm text-gray-300">
              进入 <strong className="text-brand-300">智能体</strong> 页面，点击右上角
              <code className="mx-1 rounded bg-gray-800 px-1.5 py-0.5 text-xs text-brand-300">+ 新建智能体</code>
              按钮，进入编辑器页面。
            </p>
            <ConfigCard icon="📐" title="编辑器布局" >
              <p>编辑器采用<strong>左右分栏</strong>布局：</p>
              <ul className="ml-4 mt-1 list-disc space-y-1">
                <li><strong className="text-gray-200">左侧</strong>：6 大配置模块（基础信息、模型配置、工具、知识库、技能、高级设置），以可折叠卡片形式展示</li>
                <li><strong className="text-gray-200">右侧</strong>：调试预览面板，可实时测试智能体对话、查看工具调用和思考过程</li>
              </ul>
          ...
          <Section id="engine" icon="⚙️" title="推理引擎" subtitle="Hermes vs LangGraph，如何选择？">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-lg border border-brand-500/30 bg-brand-500/5 p-4">
                <div className="mb-2 flex items-center gap-2">
                  <span className="rounded bg-brand-500/20 px-2 py-0.5 text-xs font-semibold text-brand-300">推荐</span>
                  <h4 className="text-sm font-semibold text-gray-100">Hermes 引擎</h4>
                </div>
                <ul className="space-y-1.5 text-xs text-gray-400">
                  <li>✅ 支持工具调用（ReAct 循环）</li>
                  <li>✅ 支持子代理委派（delegate_task）</li>
                  <li>✅ 支持技能注入与框架级工具</li>
                  <li>✅ 思考过程独立 SSE 事件展示</li>
                  <li>✅ 工具并行执行 + 超时保护</li>
                  <li>✅ 中间件层（护栏/验证/人工确认）</li>
                  <li>✅ 执行记录自动创建（监控可见）</li>
                </ul>
              </div>
              <div className="rounded-lg border border-gray-700 bg-gray-900/40 p-4">
                <div className="mb-2 flex items-center gap-2">
                  <span className="rounded bg-gray-700 px-2 py-0.5 text-xs font-semibold text-gray-400">兼容</span>
                  <h4 className="text-sm font-semibold text-gray-100">LangGraph 引擎</h4>
                </div>
                <ul className="space-y-1.5 text-xs text-gray-400">
                  <li>✅ 基础对话能力</li>
                  <li>✅ 工具调用（基础 ReAct）</li>
          ...
          <Section id="model" icon="🧠" title="模型配置" subtitle="选择大脑，调节性格">
            <ConfigCard icon="🔗" title="模型配置" name="model_config_id">
              ...
              <ul className="ml-4 mt-1 list-disc space-y-0.5 text-xs">
                <li><strong className="text-gray-200">Claude 3.5 Sonnet</strong>：推理强、工具调用稳定，推荐用于安全分析</li>
                <li><strong className="text-gray-200">GPT-4o</strong>：通用能力强、响应快</li>
                <li><strong className="text-gray-200">GLM-4</strong>：国产模型，中文理解优秀</li>
                <li><strong className="text-gray-200">DeepSeek</strong>：性价比高，适合高频调用</li>
              </ul>
            </ConfigCard>
            <ConfigCard icon="🌡️" title="温度" name="temperature">
              ...
              <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
                <div className="rounded border border-gray-700 bg-gray-800/40 p-2">
                  <div className="font-semibold text-gray-200">0 ~ 0.3</div>
                  <div className="text-gray-500">严谨确定。适合安全分析、工具调用、数据查询</div>
                </div>
                <div className="rounded border border-gray-700 bg-gray-800/40 p-2">
                  <div className="font-semibold text-gray-200">0.4 ~ 0.8</div>
                  <div className="text-gray-500">均衡。适合对话助手、通用问答</div>
                </div>
                <div className="rounded border border-gray-700 bg-gray-800/40 p-2">
                  <div className="font-semibold text-gray-200">0.9 ~ 2.0</div>
                  <div className="text-gray-500">创造性。适合文案生成、头脑风暴</div>
                </div>
              </div>
          ...
          <Section id="prompt" icon="💬" title="系统提示词" subtitle="定义智能体的角色和行为规范">
            <p className="text-sm text-gray-300">
              <strong className="text-gray-100">系统提示词（System Prompt）</strong>
            ...
            <ConfigCard icon="✍️" title="system_prompt" name="system_prompt">
              ...
              <ol className="ml-4 mt-1 list-decimal space-y-1 text-xs">
                <li><strong className="text-gray-200">角色定义</strong>：「你是一名 SOC 安全分析师」</li>
                <li><strong className="text-gray-200">能力边界</strong>：明确能做什么、不能做什么</li>
                <li><strong className="text-gray-200">工具使用指引</strong>：何时调用哪个工具的决策规则</li>
                <li><strong className="text-gray-200">输出格式</strong>：要求的回复结构（表格、JSON、分点等）</li>
                <li><strong className="text-gray-200">安全约束</strong>：危险操作前需确认、不泄露敏感信息</li>
              </ol>
            </ConfigCard>
            <div className="rounded-lg border border-gray-800 bg-gray-900/60 p-4">
              <div className="mb-2 text-xs font-semibold text-gray-500">示例：告警研判助手</div>
              <pre className="overflow-x-auto text-xs text-gray-300">{`你是一名 SOC 高级安全分析师，负责告警研判。
...
- 封禁操作需通过 block_ip_on_firewall 工具执行`}</pre>
            ...
            <Tip>
              系统提示词支持 <code className="rounded bg-gray-800 px-1 text-brand-300">{'{{变量名}}'}</code> 模板变量，
              会被「高级设置」中的变量值替换。例如
              <code className="ml-1 rounded bg-gray-800 px-1 text-brand-300">{'{{company_name}}'}</code> 会被替换为对应变量值。
            </Tip>
          </Section>
          ...
          <Section id="tools" icon="🔧" title="工具配置" subtitle="为智能体装配「双手」">
            ...
            <ConfigCard icon="🎨" title="工具类型标识">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                <span className="rounded border border-gray-600 bg-gray-800 px-2 py-0.5 text-gray-300">Code（灰色）</span>
                <span className="rounded border border-brand-500/40 bg-brand-500/10 px-2 py-0.5 text-brand-300">HTTP（蓝色）</span>
                <span className="rounded border border-purple-500/40 bg-purple-500/10 px-2 py-0.5 text-purple-300">框架内置（紫色）</span>
              </div>
              <ul className="ml-4 mt-2 list-disc text-xs">
                <li><strong className="text-gray-200">Code</strong>：Python 代码工具，在沙箱中执行</li>
                <li><strong className="text-gray-200">HTTP</strong>：调用外部 API，支持 GET/POST/DELETE 等方法</li>
                <li><strong className="text-gray-200">框架内置</strong>：引擎级工具（委派/澄清/规划等），不可编辑/删除</li>
              </ul>
            </ConfigCard>
            <ConfigCard icon="🚦" title="HTTP 方法颜色">
              HTTP 工具按方法区分颜色，便于安全识别：
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                <span className="rounded bg-green-500/20 px-2 py-0.5 text-green-300">GET（绿色·查询）</span>
                <span className="rounded bg-orange-500/20 px-2 py-0.5 text-orange-300">POST（橙色·创建）</span>
                <span className="rounded bg-red-500/20 px-2 py-0.5 text-red-300">DELETE（红色·删除）</span>
              </div>
            </ConfigCard>
            <ConfigCard icon="🔧" title="工具高级配置" name="tool_configs">
              展开工具项可配置：
              <ul className="ml-4 mt-1 list-disc text-xs">
                <li><strong className="text-gray-200">执行时机</strong>：自动执行 / 需用户确认（危险操作建议设为确认）</li>
                <li><strong className="text-gray-200">参数映射</strong>：将上下文变量自动映射到工具参数</li>
                <li><strong className="text-gray-200">异常处理</strong>：工具失败时的行为（继续/中止/询问用户）</li>
              </ul>
            </ConfigCard>
            <ConfigCard icon="🔍" title="工具搜索状态" name="tool_search">
              <strong className="text-gray-200">Hermes 引擎专属功能</strong>。当智能体启用的工具较多时，
              所有工具的参数定义会占用大量上下文 token。此功能将部分工具标记为
              <strong className="text-gray-200">「可延迟」</strong>，不直接注入上下文，而是按需检索。
              <ul className="ml-4 mt-2 list-disc text-xs">
                <li><strong className="text-gray-200">auto（推荐）</strong>：自动判断，可延迟工具的 schema token 超过上下文窗口阈值的 10% 时激活检索</li>
                <li><strong className="text-gray-200">on</strong>：只要有可延迟工具就激活检索</li>
                <li><strong className="text-gray-200">off</strong>：关闭，所有工具直接注入上下文（工具少时用）</li>
              </ul>
              <p className="mt-2 text-xs">
                激活后，智能体通过 <code className="text-brand-300">tool_search</code> 工具按关键词检索可用工具，
                再用 <code className="text-brand-300">tool_call</code> 执行，从而节省上下文空间。
              </p>
              <Tip type="warn">工具少于 10 个时无需开启，直接注入上下文即可。</Tip>
            </ConfigCard>
            <ConfigCard icon="📄" title="文档读取工具" name="read_document">
              <p className="text-xs">
                <code className="text-brand-300">read_document</code> 和{' '}
                <code className="text-brand-300">list_documents</code> 是内置工具，用于
                <strong className="text-gray-200">直接读取上传的未分段文档原文</strong>，而非读取知识库分段处理后的内容。
              </p>
              <ul className="ml-4 mt-2 list-disc text-xs">
                <li><strong className="text-gray-200">适用场景</strong>：需要完整文档内容时（如通读完整报告、解析整张表格、按原文顺序分析）</li>
                <li><strong className="text-gray-200">与知识库的区别</strong>：知识库做精准片段检索（RAG），返回最相关的若干分段；此工具读取整篇文档原文</li>
                <li><strong className="text-gray-200">使用方法</strong>：先调用 <code className="text-brand-300">list_documents</code> 获取文件 ID 列表，再用 <code className="text-brand-300">read_document(file_id=X)</code> 读取指定文档</li>
                <li><strong className="text-gray-200">超长保护</strong>：内容超过 20000 字符会自动截断，避免耗尽上下文</li>
              </ul>
              <Tip>
          ...
          <Section id="knowledge" icon="📚" title="知识库对接" subtitle="让智能体「读过」你的私有文档">
            ...
            <ConfigCard icon="⚙️" title="知识库配置（在知识库页面）">
              每个知识库可独立配置：
              <ul className="ml-4 mt-1 list-disc text-xs">
                <li><strong className="text-gray-200">分块模式</strong>：固定大小 / 按段落 / 按标题</li>
                <li><strong className="text-gray-200">索引模式</strong>：向量检索 / 全文检索 / 混合检索</li>
                <li><strong className="text-gray-200">Embedding 模型</strong>：文本向量化模型</li>
                <li><strong className="text-gray-200">检索设置</strong>：Top-K 数量、相似度阈值</li>
              </ul>
            </ConfigCard>
            <Tip>
          ...
          <Section id="skills" icon="🎯" title="技能注入" subtitle="用自然语言编写 SOP 流程">
            ...
            <ConfigCard icon="✍️" title="编写技能（在技能页面）">
              ...
              <pre className="mt-2 overflow-x-auto rounded bg-gray-950 p-3 text-xs text-gray-300">{`## 告警研判标准流程
...
5. 输出研判报告，包含处置建议和风险等级`}</pre>
          ...
          <Section id="middleware" icon="🧩" title="中间件配置" subtitle="安全护栏 + 人工确认 + 验证机制">
            ...
            <ConfigCard icon="🛡️" title="安全护栏（Guardrails）">
              在工具调用前拦截不安全请求：
              <ul className="ml-4 mt-1 list-disc text-xs">
                <li><strong className="text-gray-200">关键词过滤</strong>：拦截含敏感词的输入/输出</li>
                <li><strong className="text-gray-200">PII 脱敏</strong>：自动遮盖身份证号、手机号等</li>
                <li><strong className="text-gray-200">注入防护</strong>：检测 prompt injection 攻击</li>
              </ul>
            </ConfigCard>
            <ConfigCard icon="✅" title="验证机制（Verification）">
              危险操作前要求人工确认：
              <ul className="ml-4 mt-1 list-disc text-xs">
                <li><strong className="text-gray-200">自动执行</strong>：工具直接运行，无需确认</li>
                <li><strong className="text-gray-200">需用户确认</strong>：弹出确认提示，用户同意后执行</li>
            ...
            <ConfigCard icon="⏱️" title="执行时机">
              控制工具何时触发：
              <ul className="ml-4 mt-1 list-disc text-xs">
                <li><strong className="text-gray-200">自动</strong>：模型决定调用时立即执行</li>
                <li><strong className="text-gray-200">用户确认</strong>：模型决定调用后，等待用户确认</li>
              </ul>
            </ConfigCard>
          </Section>
          ...
          <Section id="advanced" icon="🎛️" title="高级设置" subtitle="精细调控智能体行为">
            ...
            <ConfigCard icon="📊" title="变量" name="variables">
              ...
              <code className="mx-1 rounded bg-gray-800 px-1 text-brand-300">{'{{key}}'}</code>
              引用。
              <ul className="ml-4 mt-1 list-disc text-xs">
          ...
          <Section id="debug" icon="🐞" title="调试与测试" subtitle="右侧面板实时验证">
            ...
            <ConfigCard icon="📊" title="流式输出">
              调试面板展示 4 类实时事件：
              <ul className="ml-4 mt-1 list-disc text-xs">
                <li><strong className="text-gray-200">🧠 思考过程</strong>：模型的推理链（可折叠）</li>
                <li><strong className="text-gray-200">💬 回复内容</strong>：逐 token 流式输出</li>
                <li><strong className="text-gray-200">🔧 工具调用</strong>：每个工具的开始/结束/结果（可折叠）</li>
                <li><strong className="text-gray-200">🌳 子代理委派</strong>：按 subagent_id 分组，可展开查看嵌套事件</li>
              </ul>
            </ConfigCard>
            <ConfigCard icon="🔄" title="状态标识">
              <ul className="ml-4 list-disc text-xs">
                <li>✅ 绿色 ✓：工具成功完成</li>
                <li>❌ 红色 ✗：工具执行出错（可查看错误详情）</li>
                <li>🔄 脉冲动画：工具正在执行中</li>
              </ul>
            </ConfigCard>
            <Tip type="tip">
          ...
          <Section id="monitor" icon="📈" title="监控与追溯" subtitle="对话记录全程可查">
            ...
            <ConfigCard icon="📜" title="执行追溯">
              「监控追溯」分组下的<strong>执行追溯</strong>页面可查看所有智能体的执行历史，
              支持按状态、时间、智能体筛选。
            </ConfigCard>
          ...
          <Section id="best-practices" icon="🏆" title="最佳实践" subtitle="来自实战的经验总结">
            ...
            <Tip type="tip">
              遇到问题？查看<strong>执行追溯</strong>中的失败记录，
          ...
          <div className="mt-12 flex items-center justify-between border-t border-gray-800 pt-6">
            <button
              onClick={() => navigate('/agents')}
              className="rounded-md border border-gray-700 px-4 py-2 text-sm text-gray-400 transition hover:bg-gray-800 hover:text-gray-200"
            >
              ← 返回智能体列表
            </button>
            <button
