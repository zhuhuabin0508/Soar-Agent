...
function ConfirmLeaveDialog({ onStay, onLeave }) {
  return h(
    ...
    h(
      'div',
      { className: 'w-96 rounded-lg border border-gray-700 bg-gray-900 p-5 shadow-2xl' },
      h(
        'div',
        { className: 'mb-2 flex items-center gap-2 text-base font-semibold text-gray-100' },
        h('span', null, '⚠️'),
        h('span', null, '未保存的修改')
      ),
      h(
        'div',
        { className: 'mb-4 text-sm leading-relaxed text-gray-400' },
      ...
      h(
        ...
        h(
          'button',
          {
            ...
            onClick: onStay,
            className:
              'rounded-md border border-gray-600 px-3 py-1.5 text-sm text-gray-300 transition hover:bg-gray-800',
          },
          '留在页面'
        ),
        h(
          'button',
          {
            ...
            onClick: onLeave,
            className:
              'rounded-md bg-danger-600 px-3 py-1.5 text-sm font-medium text-white transition hover:bg-danger-500',
          },
          '放弃修改并离开'
        )
      )
