...
// 使用语义令牌（border-input / bg-background / text-foreground / focus:ring-primary）
// 明暗主题切换零改动，避免硬编码 gray-* 在亮色主题下不可见
export const inputBaseCls =
  'rounded-md border border-input bg-background px-2.5 py-1.5 text-sm text-foreground outline-none transition placeholder:text-muted-foreground hover:border-primary/50 focus:border-primary focus:ring-1 focus:ring-primary'
...
export const hintCls = 'mt-1 text-[11px] leading-relaxed text-muted-foreground/70'

// 带可选 InfoTip 的 label 渲染辅助函数
// 将原来的 label 下方小字 hint 改为 label 右侧圆圈感叹号 tooltip
function LabelWithTip({ label, hint, required }) {
  if (!label) return null
  const requiredMark = required ? <span className="text-red-500">*</span> : null
  if (!hint) return <label className={labelCls}>{label}{requiredMark}</label>
  return (
...
export function VariableRefButton({ currentNodeId, onPick }) {
  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 max-h-[300px] w-60 overflow-y-auto rounded-md border border-border bg-secondary shadow-xl">
          ...
          {upstreamNodes.map((n) => {
            ...
            return (
              <div key={n.id} className="border-b border-border/50 last:border-0">
                <div className="truncate px-3 py-1.5 text-[11px] font-medium text-foreground/90">
                  {label}
                </div>
                {fields.map((f) => (
                  <button
...
export function Section({ title, children, hint }) {
  return (
    <div className="rounded-md border border-border bg-muted/40 p-3">
      <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-foreground/90">
        {title}
...
// 文本输入
export function TextInput({ label, value, onChange, placeholder, hint, required, maxLength }) {
  const len = (value ?? '').length
  return (
    <div>
      <LabelWithTip label={label} hint={hint} required={required} />
      <input
        className={inputCls}
        value={value ?? ''}
      ...
      />
      {maxLength && (
        <div className={`mt-1 text-right text-[11px] ${len >= maxLength ? 'text-destructive' : 'text-muted-foreground/60'}`}>
          {len}/{maxLength}
        </div>
      )}
    </div>
...
// 多行文本
export function TextArea({ label, value, onChange, rows = 4, placeholder, hint, maxLength, required }) {
  const len = (value ?? '').length
  return (
    <div>
      <LabelWithTip label={label} hint={hint} required={required} />
...
export function CheckRow({ label, checked, onChange, hint }) {
  return (
    <label className="flex cursor-pointer items-start gap-2">
      <input
        type="checkbox"
        className="mt-0.5 h-4 w-4 rounded border-border bg-secondary text-primary focus:ring-primary"
        checked={!!checked}
        onChange={(e) => onChange(e.target.checked)}
      />
      <span className="flex items-center gap-1 text-sm text-foreground/90">
        {label}
        {hint && <InfoTip text={hint} />}
      </span>
    </label>
      ...
      >
        {options.map((opt) => {
          ...
          return (
            <label
              key={val}
              className="flex cursor-pointer items-center gap-2 rounded-md border border-border bg-muted/50 px-2 py-1.5 hover:border-border"
            >
              <input
                type="checkbox"
                ...
                onChange={() => toggle(val)}
              />
              <span className="truncate text-xs text-foreground/90">{text}</span>
            </label>
          )
        })}
      </div>
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-1.5">
        {value.length === 0 && (
          <p className="text-[11px] text-muted-foreground/60">暂无配置项</p>
        )}
        {value.map((item, idx) => (
          <div key={idx} className="flex gap-1.5">
            ...
            <button
              type="button"
              onClick={() => remove(idx)}
              className="shrink-0 rounded-md border border-border px-2 text-xs text-muted-foreground hover:border-destructive hover:text-destructive"
              title="删除"
            >
              <X className="h-3 w-3" />
            </button>
      ...
      <button
        type="button"
        onClick={add}
        className="mt-1.5 w-full rounded-md border border-dashed border-border py-1 text-xs text-muted-foreground hover:border-primary hover:text-primary"
      >
        + 添加一行
      </button>
    </div>
        ...
        {value.map((item, idx) => (
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              ...
              <label
                className="flex shrink-0 cursor-pointer items-center gap-1 px-1 text-[11px] text-muted-foreground"
                title="是否必填"
              >
                <input
                  type="checkbox"
    ...
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 text-[11px]">
          <span className="text-muted-foreground/70">连接符：</span>
          {['AND', 'OR'].map((l) => (
            <button
              key={l}
              type="button"
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {value.map((item, idx) => (
          <div key={idx} className="flex flex-col gap-1 rounded-md border border-border bg-muted/40 p-1.5">
            {/* 第一行：变量名 + 引用按钮 */}
            <div className="flex gap-1.5">
              <input
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        {value.map((c, idx) => (
          <div key={idx} className="rounded-md border border-border bg-muted/40 p-2">
