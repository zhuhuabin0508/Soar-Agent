...
function DetailModal({ open, record, onClose, onUnban, onHardDelete, canUnban, unbanning, deleting }) {
  ...
  const levelMeta = LEVEL_META[record.ban_level] || { label: record.ban_level, cls: 'bg-gray-700 text-gray-300' }
  const statusMeta = STATUS_META[record.status] || { label: record.status, cls: 'bg-gray-700 text-gray-300' }
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-3">
          {fields.map((f) => (
            <div key={f.label} className="min-w-0">
              <div className="mb-1 text-xs font-medium text-gray-500">{f.label}</div>
              {f.badge ? (
                <span className={`inline-block rounded px-2 py-0.5 text-xs font-medium ${f.badge}`}>{f.value}</span>
              ) : (
                <div className={`truncate text-sm text-gray-200 ${f.mono ? 'font-mono' : ''}`} title={String(f.value)}>{f.value}</div>
              )}
            </div>
          ))}
        </div>
...
function BannedIPsPanel() {
  ...
  const handleUnban = async (record) => {
    if (!record) return
    if (!window.confirm(`确定手动解封 IP「${record.ip}」吗？解封后将无法恢复封禁状态。`)) return
    setUnbanning(true)
    try {
      await bannedIpsApi.unban(record.id)
      setDetailOpen(false); setDetailRecord(null)
      await loadAll()
    } catch (err) { window.alert(`解封失败：${err.message || err}`) }
  ...
  const handleHardDelete = async (record) => {
    ...
    if (input.trim() !== record.ip) { window.alert('输入的 IP 地址不匹配，已取消删除。'); return }
    setDeleting(true)
    try {
      await bannedIpsApi.hardDelete(record.id)
      setDetailOpen(false); setDetailRecord(null)
      await loadAll()
    } catch (err) { window.alert(`删除失败：${err.message || err}`) }
    finally { setDeleting(false) }
  }

  const handleCreate = async () => {
    if (!createForm.ip.trim()) { window.alert('请填写 IP 地址'); return }
    setCreating(true)
    try {
      ...
      await loadAll()
    } catch (err) { window.alert(`新增失败：${err.message || err}`) }
  ...
  const handleExport = async () => {
    ...
    catch (err) { window.alert(`导出失败：${err.message || err}`) }
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      ...
      await loadAll()
    } catch (err) { window.alert(`导入失败：${err.message || err}`) }
    ...
    <div className="flex flex-col w-full min-w-0">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {canUnban && (
            <button type="button" onClick={handleImportClick} disabled={importing} className="btn-secondary btn-sm">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="flex h-60 flex-col items-center justify-center gap-2 text-gray-500">
            ...
            <div className="text-sm">{search || statusFilter ? '没有匹配的封禁记录' : '暂无已封禁 IP 记录'}</div>
          </div>
        ) : (
          <div className="w-full overflow-x-auto rounded-md border border-gray-800">
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {records.map((r, idx) => {
                  const levelMeta = LEVEL_META[r.ban_level] || { label: r.ban_level, cls: 'bg-gray-700 text-gray-300' }
                  const statusMeta = STATUS_META[r.status] || { label: r.status, cls: 'bg-gray-700 text-gray-300' }
                  return (
                    <tr key={r.id} className={`border-t border-gray-800 ${idx % 2 === 0 ? 'bg-gray-900/30' : ''}`}>
                      <td className="whitespace-nowrap px-3 py-2 font-mono text-brand-300">{r.ip}</td>
                      <td className="px-3 py-2"><span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${levelMeta.cls}`}>{levelMeta.label}</span></td>
                      <td className="px-3 py-2"><span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${statusMeta.cls}`}>{statusMeta.label}</span></td>
                      <td className="px-3 py-2"><span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${(SOURCE_META[r.source] || { cls: 'bg-gray-700 text-gray-300' }).cls}`}>{(SOURCE_META[r.source] || { label: r.source || '-' }).label}</span></td>
                      <td className="whitespace-nowrap px-3 py-2 text-gray-300">{humanizeDuration(r.ban_duration)}</td>
                      <td className="whitespace-nowrap px-3 py-2 text-gray-400">{fmtExpiry(r.expired_at)}</td>
                      <td className="px-3 py-2 text-gray-300">{r.violation_count ?? 0}</td>
                      <td className="whitespace-nowrap px-3 py-2 text-gray-400">{r.region || '-'}</td>
                      <td className="max-w-[260px] truncate px-3 py-2 text-gray-400" title={r.reason || ''}>{r.reason || '-'}</td>
                      <td className="whitespace-nowrap px-3 py-2">
                        <div className="flex gap-1.5">
                          <button type="button" onClick={() => { setDetailRecord(r); setDetailOpen(true) }} className="btn-secondary btn-sm">详情</button>
                          {canUnban && r.status === 'active' && (
                            <button type="button" onClick={() => handleUnban(r)} disabled={deleting} className="btn-danger btn-sm">解封</button>
                          )}
                          {canUnban && (
                            <button type="button" onClick={() => handleHardDelete(r)} disabled={unbanning} className="btn-danger btn-sm" title="硬删除：从数据库彻底删除记录（不可恢复）">删除</button>
                          )}
                        </div>
                      </td>
                    </tr>
      ...
      >
        {importResult && (
          <div className="flex flex-col gap-3 text-sm">
            ...
            {importResult.skipped > 0 && <div className="text-gray-400">⏭️ 跳过已存在 {importResult.skipped} 条</div>}
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-500">封禁时长（秒，0=永久）</label>
