  ...
  remaining = remaining.replace(/`([^`]+)`/g, (match, code) => {
    ...
    placeholders.push(
      <code
        ...
        className="rounded bg-gray-700/80 px-1.5 py-0.5 text-[13px] font-mono text-brand-300"
      >
        {code}
      </code>
    )
    ...
    if (trimmed.startsWith('```')) {
      ...
      blocks.push(
        <pre
          ...
          className="my-2 overflow-x-auto rounded-lg border border-gray-700 bg-gray-900/80 p-3 text-[13px] leading-relaxed"
        >
          <code className="font-mono text-gray-300">{codeLines.join('\n')}</code>
        </pre>
      )
      continue
    }
    ...
    if (headingMatch) {
      ...
      blocks.push(
        React.createElement(
          ...
          {
            key: `h-${blocks.length}`,
            className: `font-bold text-white ${sizes[level - 1]} ${margins[level - 1]}`,
          },
    ...
    if (trimmed.startsWith('>')) {
      ...
      blocks.push(
        <blockquote
          ...
          className="my-2 border-l-4 border-gray-600 bg-gray-800/50 py-1.5 pl-3 pr-2 text-gray-300"
        >
    ...
    if (/^[-*+]\s+/.test(trimmed)) {
      ...
      blocks.push(
        <ul
          ...
          className="my-1.5 list-disc space-y-0.5 pl-5 text-gray-200"
        >
          {items.map((item, idx) => (
    ...
    if (/^\d+\.\s+/.test(trimmed)) {
      ...
      blocks.push(
        <ol
          ...
          className="my-1.5 list-decimal space-y-0.5 pl-5 text-gray-200"
        >
          {items.map((item, idx) => (
                ...
                <tr>
                  {headers.map((h, idx) => (
                    <th
                      ...
                      className="border border-gray-700 bg-gray-800 px-3 py-1.5 text-left font-semibold text-gray-200"
                    >
