...
export const bannedIpsApi = {
  ...
  // 导入 CSV：上传文件，需指定封禁设备 ID
  importCsv: async (file, deviceId) => {
    const formData = new FormData()
    formData.append('file', file)
    const resp = await fetch(
      `/api/v1/banned-ips/import/csv?device_id=${encodeURIComponent(deviceId)}`,
      {
        method: 'POST',
        headers: { ...authHeader() },
        body: formData,
  ...
  downloadTemplate: async () => {
    const resp = await fetch(`/api/v1/banned-ips/import/template`, {
      headers: { ...authHeader() },
    })
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = 'banned_ips_import_template.csv'
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
